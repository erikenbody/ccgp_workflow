...
rule bam2gvcf:
    ...
    input:
        ...
        bam = ancient(config['output'] + "{Organism}/{refGenome}/" + config['bamDir'] + "{sample}" + config['bam_suffix']),
    output: 
        gvcf = config['output'] + "{Organism}/{refGenome}/" + config['gvcfDir'] + "{sample}/" + "{sample}.raw.g.vcf.gz",
        gvcf_idx = config['output'] + "{Organism}/{refGenome}/" + config['gvcfDir'] + "{sample}/" + "{sample}.raw.g.vcf.gz.tbi",
        doneFile = touch(config['output'] + "{Organism}/{refGenome}/" + config['gvcfDir'] + "{sample}/" + "{sample}.done")
    resources: 
    ...
    log:
        "logs/{Organism}/{refGenome}/gvcf/{sample}.txt"
    params:
        minPrun = config['minP'],
        minDang = config['minD'],
...
rule mkDBmapfile:
    ...
    output:
        dbMapFile = config['output'] + "{Organism}/{refGenome}/" + config['dbDir'] + "DB_mapfile"
    run:
...
rule gvcf2DB:
    ...
    benchmark: "benchmarks/{Organism}/{refGenome}/gvcf2DB/DB_L{list}"
    conda:
        "../envs/bam2vcf.yml"
    shell:
...
rule DB2vcf:
    ...
    log: "logs/{Organism}/{refGenome}/DB2vcf/DB_L{list}"
    benchmark: "benchmarks/{Organism}/{refGenome}/DB2vcf/DB_L{list}"
    conda:
        "../envs/bam2vcf.yml"
    shell:
        "gatk GenotypeGVCFs "
