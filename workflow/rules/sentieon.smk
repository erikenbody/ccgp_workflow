...
rule get_fastq_pe:
    output:
        ...
        temp(config["fastqDir"] + "{Organism}/{sample}/{accession}_1.fastq.gz"),
        temp(config["fastqDir"] + "{Organism}/{sample}/{accession}_2.fastq.gz")
    log:
        "logs/{Organism}/get_fastq/{sample}/{accession}.txt"
    params:
        extra="--skip-technical"
    threads:
        res_config['get_fastq_pe']['threads']
    wrapper:
        "v1.0.0/bio/sra-tools/fasterq-dump"
