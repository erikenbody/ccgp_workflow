...
class VCF(sharder.Shardable):
    ...
    def __init__(self, path, mode='r'):
        ...
        self.fp, self.index = self.open(self.path, self.mode)
        if self.mode[0:1] == 'r':
            self.load_header()
            self.parse_header()
        else:
    ...
    def __setstate__(self, ndict):
        ...
        mode = ndict['mode']
        if mode[0:1] == 'r':
            self.fp, self.index = self.open(path, mode)
    ...
    def open(path, mode):
        if path.endswith('.gz'):
            fp = bgzf.open(path, mode)
            index = tabix.Tabix(path + '.tbi', mode)
        else:
            fp = io.open(path, mode)
            index = tribble.TribbleIndex(path + '.idx', mode)
        return (fp, index)
    ...
    def format(self, v):
        flds = [v.chrom, str(v.pos+1), v.id, v.ref, ','.join(v.alt) or '.',
            ...
            ';'.join(v.filter) or '.']
        t = [self.format_info(kv) for kv in iteritems(v.info)]
